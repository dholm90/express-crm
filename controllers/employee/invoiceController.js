...
exports.invoice_list = (req, res, next) => {
  Invoice.find({ business: req.user.business, employees: req.user._id })
    ...
    .sort({ createdAt: 'desc' })
    .exec(function (err, invoice_list) {
      if (err) {
        return next(err);
      }
      res.render('employee/invoice-list', {
        title: 'Invoices',
  ...
  async.parallel(
    {
      ...
      invoice(callback) {
        Invoice.findOne({ _id: req.params.id, business: req.user.business }).populate([{ path: 'loggedHours', model: HourLog, populate: [{ path: 'employee', ref: 'User' }] }, { path: 'materials.material', model: Material }, { path: 'employees', model: User }, { path: 'client', model: Client }, { path: 'services.service', model: Service }, 'images']).exec(callback);
      }
    },
    async (err, results) => {
      if (err) {
      ...
      await res.render('employee/invoice-detail', {
...
exports.add_material = [
  ...
  async (req, res, next) => {
    ...
    await invoice.save();
    await res.redirect(invoice.url_employee);
  }
]
...
exports.create_invoice_get = (req, res, next) => {
  Client.find({ business: req.user.business })
    ...
    .exec(function (err, clients) {
      ...
      res.render('employee/invoice-form', {
        title: 'New Invoice',
        parent_page: 'Invoices',
        layout: './layouts/employee',
        clients,
...
exports.create_invoice_post = [
  ...
  (req, res, next) => {
    ...
    const invoiceData = new Invoice({
      ...
      res.redirect(`/employee/invoice-list`);
    })
  }
]
    ...
    if (!errors.isEmpty()) {
      async.parallel(
        ...
        (err, results) => {
          ...
          res.render('employee/invoice-detail', {
            title: `Invoice: ${results.invoice.title}`,
            parent_page: 'Invoices',
            layout: './layouts/employee',
            invoice: results.invoice,
