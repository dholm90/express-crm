...
getTotal = async (req, res, next, subMonth) => {
  ...
  let invoices = await Invoice.find({
    ...
    "$expr": { "$eq": [{ "$month": "$completedOn" }, d.getMonth() - subMonth + 1] }
  }).populate([
  ...
  for (let invoice of invoices) {
    ...
    tally += parseFloat(invoice.total);
    // console.log(tally)
  }
  invoices = {};
  // console.log(tally)
...
getLastTotal = async (req, res, next) => {
  const d = new Date()
  d.setMonth(d.getMonth() - 1);
  // let lastMonth = d.getMonth();
  let invoices = await Invoice.find({
    business: req.user.business,
    "$expr": { "$eq": [{ "$month": "$completedOn" }, d.getMonth()] }
  }).populate([
    { path: 'materials.material', model: Material },
    { path: 'services.service', model: Service },
  ])
  ...
  async.parallel(
    {
      ...
      async getData() {
        ...
        for (i = 0; i < 12; i++) {
          let total = await getTotal(req, res, next, i)
          if (currentMonth == - 1) {
            currentMonth = 11;
          ...
          let invoices = await Invoice.find({
            business: req.user.business,
            "$expr": { "$eq": [{ "$month": "$completedOn" }, d.getMonth() - i + 1] }
          }).populate([
            { path: 'materials.material', model: Material },
            { path: 'services.service', model: Service },
        ...
        // console.log(lastTwelveTotals)
        const thisMonthTotal = await getTotal(req, res, next, 0);

        // console.log(date.getMonth)
        const lastMonthTotal = await getLastTotal(req, res, next);
