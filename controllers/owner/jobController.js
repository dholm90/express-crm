...
exports.job_list = (req, res, next) => {
  Job.find({ business: req.user.business })
  ...
  async.parallel(
    {
      ...
      // hours(callback) {
      //   HourLog.find({business: req.user.business,})
      // },
      job(callback) {
    ...
    async (err, results) => {
      ...
      await res.render('dashboard/job-detail', {
...
exports.create_job_post = [
  ...
  (req, res, next) => {
    ...
    const jobData = new Job({
      ...
      res.redirect(`/dashboard/job-list`);
    })
  }
]
    ...
    if (!errors.isEmpty()) {
      async.parallel(
        ...
        (err, results) => {
          ...
          res.render('dashboard/job-detail', {
            title: `Job: ${results.job.title}`,
            parent_page: 'Jobs',
            layout: './layouts/dashboard',
            job: results.job,
