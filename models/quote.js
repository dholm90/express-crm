...
QuoteSchema.virtual("url").get(function () {
  ...
  return `/dashboard/quote/${this._id}`;
});

QuoteSchema.virtual("url_employee").get(function () {
  // We don't use an arrow function as we'll need the this object
  return `/employee/quote/${this._id}`;
});

QuoteSchema.virtual("jobStart_formatted").get(function () {
  return DateTime.fromJSDate(this.jobStart).toUTC().toFormat('yyyy-MM-dd');
});

QuoteSchema.virtual("jobStart_formatted_weekday").get(function () {
  return DateTime.fromJSDate(this.jobStart).toUTC().toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY);
});

QuoteSchema.virtual("jobEnd_formatted").get(function () {
  return DateTime.fromJSDate(this.jobEnd).toUTC().toFormat('yyyy-MM-dd');
});

QuoteSchema.virtual("jobEnd_formatted_weekday").get(function () {
  return DateTime.fromJSDate(this.jobEnd).toUTC().toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY);
});

QuoteSchema.virtual('services_subtotal').get(function () {
...
QuoteSchema.virtual('materials_subtotal').get(function () {
